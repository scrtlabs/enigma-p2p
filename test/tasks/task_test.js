...
const contractAddress = "ae2c488a1a718dd9a854783cc34d1b3ae82121d0fc33615c54a290d90e2b02b3";
const preCode =
...
it("#1 Should test computeTask", function(done) {
  ...
  assert.strictEqual(constants.TASK_STATUS.UNVERIFIED, task.getStatus(), "not unverified");
  assert.strictEqual(
    constants.TASK_STATUS.IN_PROGRESS,
    task.setInProgressStatus().getStatus(),
    "status not in progress"
  );
  assert.strictEqual(constants.TASK_STATUS.SUCCESS, task.setSuccessStatus().getStatus(), "status not success");
  assert.strictEqual(constants.TASK_STATUS.FAILED, task.setFailedStatus().getStatus(), "status not failure");
