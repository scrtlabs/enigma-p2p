...
const userTaskId =
  "ae2c488a1a718dd9a854783cc34d1b3ae82121d0fc33615c54a290d90e2b02b3";
const encryptedArgs =
...
it("#1 Should test computeTask", function(done) {
  ...
  assert.strictEqual(
    ...
    task.setSuccessStatus().getStatus(),
    "status not success"
  );
  assert.strictEqual(
    constants.TASK_STATUS.FAILED,
    task.setFailedStatus().getStatus(),
    "status not failure"
  );
...
it("#5 Should test OutsideTask", function(done) {
  ...
  let outsideTask = OutsideTask.buildTask(
  ...
  assert.strictEqual(
    ...
    `taskId dont ${outsideTask.getTaskId()} match original ${
      resultRawObj.taskId
    }`
  );
  assert.strictEqual(
