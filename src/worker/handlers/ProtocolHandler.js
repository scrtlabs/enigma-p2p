...
class ProtocolHandler extends EventEmitter {
  ...
  handleTopic(params, message) {
    ...
    if (
      topicIDs.length > 0 &&
      params.worker.getProtocolHandler().policy.isValidTopic(topicIDs[0])
    ) {
  ...
  onPeerDiscovery(nodeBundle, params) {
    ...
    if (!params.worker.isConnected(params.peer.id.toB58String())) {
      params.worker
        .getProtocolHandler()
        .notify({ notification: NOTIFICATION["DISCOVERED"], params: params });
    }
  }
          ...
          conn.getPeerInfo((err, peerInfo) => {
            if (err) {
              worker
                .getProtocolHandler()
                ._logger.error(
                  "[-] err retrieving peer info from connection on handshake " +
                    err
                );
              return;
            }
  ...
  onPeerDisconnect(nodeBundle, params) {
    params.worker
      .getProtocolHandler()
      ._logger.info("peer disconnected from " + params.peer.id.toB58String());
  }
  /**
  ...
  onLocalStateExchange(nodeBundle, params) {
    ...
    params.worker
      .getProtocolHandler()
      ._logger.debug("[LOCAL_STATE] got local state request from remote peer.");
    self.notify({
      ...
      onResponse: (err, tips) => {
        if (err) {
          params.worker
            .getProtocolHandler()
            ._logger.debug(`[ERROR] get local tips ${err}.`);
          return;
        }
        pull(
          params.connection,
  ...
  onTaskResultPublish(params, message) {
    ...
    const self = params.worker.getProtocolHandler();
    self.notify({
      notification: NOTIFICATION.RECEIVED_NEW_RESULT,
      params: message
    });
  }
}
