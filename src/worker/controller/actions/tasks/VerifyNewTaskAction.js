    ...
    this._controller.execCmd(constants.NODE_NOTIFICATIONS.REGISTRATION_PARAMS, {
      onResponse: async (err, regParams) => {
        ...
        if (this._controller.hasEthereum()) {
          isVerified = false;
          try {
            const currentBlockNumber = await ethUtils.getEthereumBlockNumber(
              this._controller
                .ethereum()
                .api()
                .w3()
            ...
            this._controller
              .logger()
              .error(
                `[VERIFY_NEW_TASK] an exception occurred while trying to verify task ${unverifiedTask.getTaskId()} = ${err}`
              );
          }
        }
