...
describe('actions_tests',()=>{
  ...
  it('#3 AnnounceContent action', async function(){
    ...
    return new Promise(async (resolve)=>{
      ...
      let blob = [22,33,44,55,66,77,88,99,100,101,102,103,104,105,106,107,108,109,110,111,112,113,113,115,116];
      // 2. turn it into eng cid
      let engCid = EngCid.createFromByteArray(blob);
      // 3. announce the content
      let failedCids = await peerController.getNode().asyncExecCmd(
          constants.NODE_NOTIFICATIONS.ANNOUNCE_ENG_CIDS, {engCids : [engCid]});
      ...
      assert.strictEqual(peerController.getNode().getSelfB58Id(),providers[0].id.toB58String(), `peerId != providerId`);
