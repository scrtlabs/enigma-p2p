...
describe("Ethereum tests", function() {
  ...
  it("Register a worker, deposit, withdraw and deploy a secret contract using the BUILDER ", async function() {
    ...
    return new Promise(async function(resolve) {
      ...
      const secretContractAddress = utils.remove0x(
        api.w3().utils.randomHex(32)
      ); // accounts[5];
      const secretContractAddress2 = utils.remove0x(
        api.w3().utils.randomHex(32)
      ); // accounts[6];
      ...
      eventSubscribe(
        api,
        constants.RAW_ETHEREUM_EVENTS.DepositSuccessful,
        {},
        getEventRecievedFunc(
          constants.RAW_ETHEREUM_EVENTS.DepositSuccessful,
          result => {
      ...
      eventSubscribe(
        api,
        constants.RAW_ETHEREUM_EVENTS.WithdrawSuccessful,
        {},
        getEventRecievedFunc(
          constants.RAW_ETHEREUM_EVENTS.WithdrawSuccessful,
          result => {
      ...
      assert.strictEqual(
        constants.RAW_ETHEREUM_EVENTS.Registered in events,
        true
      );
      events = await api.deposit(workerAddress, depositValue, {
        from: workerAddress
      });
      assert.strictEqual(
        constants.RAW_ETHEREUM_EVENTS.DepositSuccessful in events,
        true
      );
      ...
      assert.strictEqual(
        workerState.status,
        constants.ETHEREUM_WORKER_STATUS.LOGGEDIN
      );
      ...
      events = await api.deploySecretContract(
        secretContractAddress,
        codeHash,
        codeHash,
        initStateDeltaHash,
      ...
      assert.strictEqual(
        constants.RAW_ETHEREUM_EVENTS.SecretContractDeployed in events,
        true
      );
      ...
      const observedAddressesArray1 = await api.getSecretContractAddresses(
        0,
        1
      );
      ...
      await api.deploySecretContractFailure(
        taskId1,
        codeHash,
        gasUsed,
        signature,
      ...
      assert.strictEqual(
        workerState.status,
        constants.ETHEREUM_WORKER_STATUS.LOGGEDOUT
      );
  ...
  it("Register, login, deploy secret contract, create tasks and commit receipts/failure using the BUILDER ", async function() {
    ...
    return new Promise(async function(resolve) {
      ...
      const config = {
        enigmaContractAddress: builder1.enigmaContractAddress,
        enigmaContractABI: builder1.enigmaContractABI
      };
      const builder2 = new EnigmaContractAPIBuilder();
      let res2 = await builder2
        .useDeployed(config)
        .setEthereumAddress(workerAddress)
        .build();
      ...
      const mock_taskId =
        "0xf29647ec8920b552fa96de8cc3129b5ba70471b190c8ec5a4793467f12ad84e9";
      ...
      await api.createDeploymentTaskRecord(
        inputsHash,
        gasLimit,
        gasPrice,
        firstBlockNumber,
          ...
          result => {
            switch (count) {
              case 0:
                ...
                assert.strictEqual(
                  result.optionalEthereumData,
                  optionalEthereumData
                );
                assert.strictEqual(
                  result.optionalEthereumContractAddress,
                  optionalEthereumContractAddress
                );
