...
class VerifyAndStoreResultAction {
  ...
  async execute(params) {
    ...
    if (!err) {
        ...
        try {
          ...
          if (outsideTask) {
            if (verifyError instanceof errors.TaskEthereumFailureErr) {
              outsideTask.setStatus(
                constants.TASK_STATUS.FAILED.FAILED_ETHEREUM_CB
              );
            }
      ...
      this._controller
        .logger()
        .debug(`[VERIFY_AND_STORE_RESULT] finished : is_err ?  ${error}`);
      if (optionalCallback) {
      ...
      this._controller
        .logger()
        .debug(`[VERIFY_AND_STORE_RESULT] finished with an error:  ${err}`);
      if (optionalCallback) {
