...
const address2 = '0123456789ABcdeF0123456789AbcDEF00000001';
...
it('should create EngCID from network', async function(){
  let en = e1.encodeToNetwork()
  assert.deepEqual(en, Buffer.from([217,51,122,52,51,65,97,71,69,121,57,101,88,85,83,115,78,54,49,69,53,88,84,53,87,53,66,100,112,90,109,86,118,120,89,115,71,74,82,84,111,86,74,51,54,102,102,52,52,70,112,121,117]));
  let e2 = EngCID.createFromNetwork(en);
  assert(e1.equalEngCID(e2));
})

it('should fail to create from network', async function(){
  let e2 = EngCID.createFromNetwork('not a b58 encoded array');
...
it('should equalEngCID and equalKeccack256', async function(){
  let e2 = EngCID.createFromKeccack256(hash.substring(2,66));
  assert(e1.equalEngCID(e2));
  assert(e1.equalKeccack256(hash));
})

it('should fail to equalKeccack256', async function(){
  let f = e1.equalKeccack256('not a valid hash')
  assert.strictEqual(f, false)
})

it('should not equalCID', function(){
  let e2 = CIDUtil.createCID(hash.substring(2,66))
  assert(!e1.equalEngCID(e2));
})
...
it('should fail to create EngCID', async function(){
  let f = EngCID.createFromKeccack256('not a valid hash');
  assert.strictEqual(f, null);
})

it('should hash', async function(){
  let h = StateUtils.kecckak256Hash('Hello World')
...
it('should int -> bytes -> int', async function(){
  let b = StateUtils.intTo4BytesArr(47);
  assert.strictEqual(StateUtils.bytesArrToInt(b), 47)
})

it('should tuple -> key -> tuple', async function(){
  let a = hash.toLowerCase().substring(2,66)
  let k = StateUtils.toBytesKey(StateUtils.hexToBytes(a), 7)
