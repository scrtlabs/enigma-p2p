...
function prepareContractAndStates() {
  return new Promise(async (resolve)=>{
    ...
    peerMockCore.setReceiverTips(DB_PROVIDER);
    ...
    let extraPeers = 7;
    let peers =  [];
    for (let i = 0; i < extraPeers; ++i) {
      peers[i] = testBuilder.createNode(noLoggerOpts.pOpts)
    }
    ...
    await peerController.getNode().asyncTryConsistentDiscovery();
    let hc = await peerController.healthCheck();
    console.log(`health check: ${JSON.stringify(hc)}`);
